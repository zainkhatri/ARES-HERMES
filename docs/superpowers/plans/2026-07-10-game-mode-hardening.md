...
- Known values: VM IP 192.168.20.215, Sunshine creds zainkhatri/saima (PIN API: `curl -sk -u zainkhatri:saima -X POST https://192.168.20.215:47990/api/pin -d '{"pin":"XXXX","name":"pc"}'`), VDD config `C:\VirtualDisplayDriver\vdd_settings.xml`, loan flag `/mnt/nvme/PROMETHEUS/PROJECTS/ARES-DASHBOARD/.gpu-on-loan`, dashboard curl-with-login pattern in the launch plan Task 7.
