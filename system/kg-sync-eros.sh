#!/bin/bash
# kg-sync-eros.sh — runs on the ARES host. Ships the stdlib mnemosyne package to
...
rsync -a --delete -e "$SSH" "$MNEMO/mnemosyne" "$ERO:$REMOTE/" || { echo "kg-sync-eros: rsync push failed"; exit 1; }
...
$SSH "$ERO" "export HOME=/root; rm -f $REMOTE/eros_kg.db; \
  for r in $EROS_ROOTS; do [ -d \"\$r\" ] || continue; \
    ...
    PYTHONPATH=$REMOTE python3 -m mnemosyne.cli reindex \"\$r\" --box EROS; done" \
