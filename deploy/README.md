# Dashboard deploy — ZEUS instance
...
| Thing | Value |
|---|---|
| Box hostname | `zeus` (was ZEUS) |
| Tailscale IP | `100.100.29.36` |
| Dashboard brand | `ZEUS` (set by `HOST_BRAND` in the systemd unit) |
| Deploy dir | `/home/zain/ARES-DASHBOARD` (yes, "ARES" — it's the shared repo) |
| This dashboard | **port 8890**, Tailscale-only bind |
| The OLD dashboard | `zeus-dash.service` on **port 8888** (still running; decommission when ready) |

## The service (runs WITHOUT sudo — it's a user service)
```
export XDG_RUNTIME_DIR=/run/user/$(id -u)
systemctl --user status  zeus-dashboard
systemctl --user restart zeus-dashboard
journalctl --user -u zeus-dashboard -n 50 --no-pager
```
Unit file: `~/.config/systemd/user/zeus-dashboard.service`
...
Tailscale + MagicDNS — Mac/phone/etc; NOT public, NOT Funnel):
    ...
    https://zeus.tail3045df.ts.net:8443/
...
Note: the box's DNS name is `zeus` in Tailscale (renaming to `zeus` would move
...
peer's dashboard. Config per box (ARES `.env`, ZEUS systemd unit):

    PEER_URL   = http://<peer tailscale IP>:<port>   # ARES->ZEUS :8890, ZEUS->ARES :8080
    PEER_NAME  = ZEUS | ARES
...
## Redeploy (from ARES)
```
/mnt/nvme/PROMETHEUS/PROJECTS/ARES-DASHBOARD/deploy/sync-to-zeus.sh --restart
```
